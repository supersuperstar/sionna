...
def saveFig(info,title,tau_true,tau_est,crb,tgpos):
    ...
    if cell_pos is None:
        tgpos_x = x/cell_size-(tgpos[0]-map_center[0]+x/2)/cell_size
        tgpos_y = (tgpos[1]-map_center[1]+y/2)/cell_size
...
def main():
    ...
    for info in scene_info:
        ...
        if x is None or y is None or cell_size is None or map_center is None:
            ...
            env_title = f"./Data/{scene_name}/{num_samples}-{ray_type}-{max_depth}-{cell_num}-{cell_pos[0]}-{cell_pos[-1]}"
        else:
        ...
        for i,tgpos in enumerate(tgposes):
            ...
            if info.get("cell_pos") is not None:
                title = f"./Data/{scene_name}/{tgname}/{tgpos}/{tgv}/{num_samples}-{ray_type}-{max_depth}-{cell_num}-{cell_pos[0]}-{cell_pos[-1]}"
            else:
